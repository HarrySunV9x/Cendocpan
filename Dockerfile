...
RUN tar -xvf /home/panda3d-1.10.9.tar.gz -C /home
RUN yum clean all
RUN yum makecache
RUN yum -y upgrade
RUN yum -y install gcc-toolset-11 git python39 python39-devel rpm-build mesa-libGL-devel epel-release
...
RUN cd /home/panda3d-1.10.9 && python3 makepanda/makepanda.py --everything --installer --no-egl --no-gles --no-gles2 --no-opencv

RUN git clone https://github.com/lettier/3d-game-shaders-for-beginners.git /home/3d-game-shaders-for-beginners
RUN cd /home/3d-game-shaders-for-beginners/demonstration && \
    g++ -c src/main.cxx \
    ...
    g++ 3d-game-shaders-for-beginners.o \
